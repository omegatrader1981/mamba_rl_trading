...
class FuturesTradingEnv(gym.Env):
    ...
    def _validate_inputs(self, df, feature_cols):
        ...
        if not all(c in df.columns for c in ['open', 'high', 'low', 'close']):
            raise ValueError("DataFrame needs OHLC columns.")
        if not all(c in df.columns for c in feature_cols):
            raise ValueError("DataFrame missing specified feature columns.")
    ...
    def _get_info(self) -> dict:
        info = {
            "step": self.current_step, "timestamp": self.df.index[self.current_step],
            "balance": self.account.balance, "contracts_held": self.account.contracts_held,
            "position": self.account.position, "entry_price": self.account.entry_price,
            "market_price": self._get_current_price(),
            "portfolio_value": self.account.portfolio_value(self._get_current_price()),
            "trade_occurred_this_step": self._trade_occurred_this_step,
            "realized_pnl_on_close": self._info_realized_pnl_on_close,
            "consecutive_holds": self._consecutive_holds, "steps_in_trade": self.account.steps_in_trade
        }
