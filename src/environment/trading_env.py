# <<< SAC-READY VERSION: Integrates the new shaped reward scheme. >>>
...
class FuturesTradingEnv(gym.Env):
    ...
    def __init__(self, df: pd.DataFrame, feature_cols: List[str], cfg: DictConfig):
        ...
        log.info("Initializing FuturesTradingEnv (SAC-Ready Version)...")
        ...
        self.account = AccountState(initial_balance=float(self.env_cfg.initial_balance), point_value=float(self.env_cfg.point_value), commission_per_contract=float(self.env_cfg.commission_per_contract))
    ...
    def step(self, action: int) -> Tuple[Dict[str, np.ndarray], float, bool, bool, dict]:
        ...
        if self.reward_scheme == 'sac_shaped':
            ...
            reward = calculate_sac_shaped_reward(unrealized_pnl_change=pnl_component, realized_pnl_on_close=realized_pnl, action_taken=action, position_was_flat=position_was_flat, trade_closed_this_step=trade_closed, volatility=volatility_feature, activity_bonus_scale=self.activity_bonus_scale, hold_penalty_scale=self.hold_penalty_scale, win_bonus_scale=self.win_bonus_scale, loss_penalty_scale=self.loss_penalty_scale)
            terminated_by_penalty = False
        else:
            reward, terminated_by_penalty = calculate_pnl_and_shaping_reward(pnl_component, action, self._is_invalid_action, self._consecutive_holds, self._trade_occurred_this_step, self.hold_penalty_scale, self.activity_reward_scale, self.max_consecutive_holds_limit, self.excessive_hold_penalty)
        self.current_step += 1
