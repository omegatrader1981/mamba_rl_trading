...
class FuturesTradingEnv(gym.Env):
    ...
    def _map_continuous_action_to_discrete(self, continuous_action: np.ndarray) -> int:
        action_val = continuous_action[0]
        if self.account.position != 0:
            if (self.account.position == 1 and action_val < -self.action_threshold) or \
               (self.account.position == -1 and action_val > self.action_threshold):
                return self._ACTION_EXIT_POSITION
            else:
                return self._ACTION_HOLD
        else:
            if action_val > self.action_threshold:
                return self._ACTION_ENTER_LONG
            elif action_val < -self.action_threshold:
                return self._ACTION_ENTER_SHORT
            else:
                return self._ACTION_HOLD
    ...
    def _execute_agent_trade(self, action: int):
        ...
        if action == self._ACTION_HOLD: return
        if action in [self._ACTION_ENTER_LONG, self._ACTION_ENTER_SHORT]:
            if self._is_trading_forbidden():
                self._is_invalid_action = True
                return
            if self.account.position == 0:
                order_side = 1 if action == self._ACTION_ENTER_LONG else -1
            ...
            return
        if action == self._ACTION_EXIT_POSITION:
            if self.account.position != 0:
    ...
    def step(self, action: np.ndarray) -> Tuple[Dict[str, np.ndarray], float, bool, bool, dict]:
        discrete_action = self._map_continuous_action_to_discrete(action)
        ...
        if self.reward_scheme == 'sac_shaped':
            ...
            reward = calculate_sac_shaped_reward(
                ...
                action_taken=discrete_action, position_was_flat=position_was_flat,
            ...
            reward, terminated_by_penalty = calculate_pnl_and_shaping_reward(
                pnl_component, discrete_action, self._is_invalid_action, self._consecutive_holds,
