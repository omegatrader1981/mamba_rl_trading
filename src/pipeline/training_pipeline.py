...
def train_and_evaluate_model(cfg: DictConfig, df_train_s: pd.DataFrame, df_val_s: pd.DataFrame, df_test_s: pd.DataFrame, feature_cols: List[str], scaler: object):
    ...
    if cfg.optimization.get("enabled", False):
        ...
        if not best_params: raise RuntimeError("Hyperparameter optimization failed.")
    ...
    if best_params:
        class DummyTrial:
            ...
            def suggest_float(self, name, low, high, log=False): return self.params.get(name, (low + high) / 2)
            def suggest_categorical(self, name, choices): return self.params.get(name, choices[0])
            def suggest_int(self, name, low, high): return self.params.get(name, (low + high) // 2)
        ...
        if agent_type == 'sac': final_model = SAC("MlpPolicy", final_env, verbose=1, device="auto")
        else: final_model = PPO("MlpPolicy", final_env, verbose=1, device="auto")
