# <<< CORRECTED: Samples all trial-specific hyperparameters, including activity_reward_scale. >>>
...
class Objective:
    ...
    def __call__(self, trial: optuna.Trial) -> float:
        ...
        try:
            # 1. Sample feature & environment parameters for this trial
            feature_params = {
            ...
            # 2. Setup environment with trial-specific parameters
            ...
            # <<< THE PRIMARY FIX IS HERE: Sample the missing hyperparameter >>>
            activity_reward_scale = trial.suggest_float(
                "activity_reward_scale", 
                self.cfg.optimization.activity_reward_scale_min, 
                self.cfg.optimization.activity_reward_scale_max
            )

            trial_env_cfg = self.cfg.environment.copy()
            OmegaConf.update(trial_env_cfg, "lookback_window", lookback_window)
            OmegaConf.update(trial_env_cfg, "activity_reward_scale", activity_reward_scale)
